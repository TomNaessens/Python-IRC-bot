...
def addquote(conn, msg):
    if len(msg.text.split()) > 1:
        ...
        rowid = sqlite.set('INSERT INTO irc_quote (quote) VALUES (%s)', (quote,))
...
def delquote(conn, msg):
    if msg.user == settings.irc_OWNER or utils.isadmin(conn, msg):
        if len(msg.text.split()) > 1 and msg.text.split()[1].isdigit():
            rowid = sqlite.set('DELETE FROM irc_quote WHERE id=%s', (msg.text.split()[1],))
...
def quote(conn, msg):
    ...
    if len(parts) == 1: # Random quote
        rows, count = sqlite.get('SELECT * FROM irc_quote ORDER BY RAND()', ())

    elif parts[1].isdigit(): # Haal quote nummer zoveel op
        rows, count = sqlite.get('SELECT * FROM irc_quote WHERE id=%s', (parts[1],))

    else:
        rows, count = sqlite.get('SELECT * FROM irc_quote WHERE quote LIKE %s ORDER BY RANDOM()', ("%"+parts[1]+"%",))
    if count != 0:
